...
import { FaFire, FaCheckCircle, FaTimesCircle, FaDumbbell, FaCalendarAlt, FaTrophy, FaChartLine } from 'react-icons/fa';
import './gym.css';
...
export const GymHabit: React.FC = () => {
  ...
  return (
    <div className="gym-habit-container">
      {/* Header */}
      ...
      <div className="action-buttons">
        ...
        >
          <FaCheckCircle />
          {getDayStatus(today) === 'completed' ? '¡Hoy ya entrené!' : 'Entrené hoy'}
        </button>
            ...
            .map((record, index) => (
              <div key={index} className="activity-item">
                ...
                <span className="activity-text">
                  Entrenaste el {formatDate(record.date, "d 'de' MMMM")}
                </span>
              </div>
            ))}
          {records.filter(r => r.completed).length === 0 && (
            <p className="no-activity">Aún no hay entrenamientos registrados. ¡Comienza hoy!</p>
          )}
        </div>
      </div>
    </div>
