...
import { FaEdit, FaTrash, FaCheck, FaTimes, FaCalendarAlt } from 'react-icons/fa';
import { useHabits } from '../../hooks/useHabits';
import { useAuth } from '../../context/AuthContext';
import Swal from 'sweetalert2';
import './HabitDetail.css';

export const HabitSimple: React.FC = () => {
  const { habitId } = useParams<{ habitId: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();
  const { habits, updateHabit, deleteHabit, toggleHabitCompletion, getHabitRecords } = useHabits();
    ...
    if (habitId && habits.length > 0) {
      ...
      if (foundHabit) {
        ...
        setEditForm({
          ...
          color: foundHabit.color || '#000000',
          is_glossy: foundHabit.is_glossy || false,
          frequency: foundHabit.frequency || 'Daily',
          time_preference: foundHabit.time_preference || 'All day'
        });
        loadHabitRecords(foundHabit);
      }
    }
  }, [habitId, habits]);

  const loadHabitRecords = async (habitData: any) => {
    try {
      setLoading(true);
  ...
  const handleDelete = async () => {
    const result = await Swal.fire({
      ...
      text: `¿Seguro que quieres eliminar "${habit?.name}"?`,
      icon: 'warning',
